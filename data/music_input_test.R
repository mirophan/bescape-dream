...
scfilelist <- c('../unfiltered_data/crc_tumor_eset.RDS')
...
do_scdc <- function(expression_path, dataset_name){
    ...
    out.prop <- music_prop(bulk.eset = bulk, sc.eset = scdata[[2]], clusters = celltypevar, samples=samplevar)
...
#    ens <- SCDC_ENSEMBLE(bulk.eset = bulk, sc.eset.list = scdata, ct.varname = celltypevar, sample = samplevar, truep = NULL, ct.sub =  celltypesel, search.length = 0.01, grid.search = T)
    ...
    t<-as.data.frame(out.frac,rownames=TRUE)
    if(!("monocytes" %in% colnames(t))){
        t$monocytes<-0}   
    if(!("neutrophils" %in% colnames(t))){
        t$neutrophils<-0}
    out.frac<-as.matrix(t)
    rm(t)
    
    result_matrix <-  as.data.frame(out.frac)    
...
readr::write_csv(combined_result_df, "output/predictions_unionCells_ds1ds2_coarsed.csv")
