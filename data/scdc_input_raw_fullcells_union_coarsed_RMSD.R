...
do_scdc <- function(expression_path, dataset_name){
    ...
    paramsel="RMSD_Y"
    result_matrix <-  as.data.frame(wt_prop(ens$w_table[which(rownames(ens$w_table)==paramsel), 1:nscdata], ens$prop.only))
    write.table(ens$w_table,file="./output/ds1ds2weights.txt",append=TRUE,quote=FALSE)
...
readr::write_csv(combined_result_df, "output/predictions_unionCells_ds1ds2_coarsed_RMSD.csv")
