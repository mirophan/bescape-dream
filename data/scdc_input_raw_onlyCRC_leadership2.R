...
scfilelist <- c('../unfiltered_data/crc_tumor_eset_update.RDS')
...
input_df <- readr::read_csv("./input/leaderboard2_truncated/input.csv")
...
do_scdc <- function(expression_path, dataset_name){
    ...
    ens <- SCDC_prop(bulk.eset = bulk, sc.eset = scdata[[1]], ct.varname = celltypevar, sample = samplevar, truep = NULL, ct.sub =  celltypesel, search.length = 0.01, grid.search = T)
...
result_dfs <- do_scdc(expression_paths[1], dataset_names[1]) 
...
readr::write_csv(combined_result_df, "output/predictions_onlyCRC_leadership2.csv")
